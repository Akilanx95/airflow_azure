...
dag = DAG(
    ...
    description='DAG to move data from Silver to Gold zone',
...
def fetch_metadata():
    ...
    cursor.execute("SELECT * FROM dbo.meta_data WHERE is_active = 1 AND Layer = 'SilverToGold'")
...
def run_databricks_notebook(metadata):
    ...
    notebook_task = {
        "notebook_path": f"workspace/IDE/ETL/DataIngestion/{metadata['notebook_name']}",
        "base_parameters": {
